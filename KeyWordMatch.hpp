...
public:
  explicit KeyWordMatch( T initializer, size_t size );  // initializer is some default value of T
  ~KeyWordMatch();
  void ClearPatterns();
  void AddPattern( const std::string &sPattern, T object );  // do patterns need to be pre-sorted?
  size_t GetNodeCount() { return m_vNodes.size(); };
  size_t GetPatternCount() { return m_cntPatterns; };
  T FindMatch( const std::string &sMatch ) const;
protected:
	T m_Initializer;
  struct structNode {
...
template<typename T> KeyWordMatch<T>::~KeyWordMatch() {
  m_vNodes.clear();
}
...
template<typename T> T KeyWordMatch<T>::FindMatch( const std::string &sPattern ) const {
